  ...
  if (domDoc) {
    ...
    if (fonts) {
      if (
        ...
        fonts.forEach((font: Font) => markLoaded(font)); // Pretend we loaded the fonts to allow printing to continue
        logMessages({
          messages: [
    ...
    const originalSelects = (contentNode as Element).querySelectorAll(
      selectSelector
    );
